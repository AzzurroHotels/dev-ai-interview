...
async function submitInterview() {
  ...
  try {
    ...
    if (practiceClip) {
      ...
      await supabase.from("cbit_interviews").update({
    ...
    for (let i = 0; i < recordedClips.length; i++) {
      ...
      const path = `cbit-interviews/${interviewId}/q${String(i + 1).padStart(2, "0")}_${c.question_id}.${ext}`;
    ...
    const { error: fnErr } = await supabase.functions.invoke("cbit-send-interview-email", {
      body: {
        interview_id: interviewId,
    ...
    try {
      if (interviewId) await supabase.from("cbit_interviews").update({ status: "failed" }).eq("id", interviewId);
    } catch {}
    throw e;
  } finally {
    // Stop camera
