...
serve(async (req) => {
  ...
  try {
    ...
    const emailHtml = `
      <div style="font-family:sans-serif; max-width:640px; margin:0 auto; color:#333;">
        ...
        <div style="padding:16px; background:#f0f0f0; border-radius:0 0 8px 8px; text-align:center;">
          <p style="margin:0; color:#999; font-size:12px;">Sent by CBIT AI Interview Portal • devinterview@azzurrohotels.com</p>
        </div>
      </div>
    `;
    ...
    const resendRes = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        from: "CBIT Interviews <devinterview@azzurrohotels.com>",
